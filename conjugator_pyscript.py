...
def user_pos_confirmation(event):
    ...
    if possible[0] == None: confirmation = "I don't know the word '{0}'... Can you double check the Nishnaabemwin Online Dictionary?".format(lemma)
        ...
        confirmation = "The word '{0}' is a {2}. You can go straight to the {2} section in step 2. If you want more information about the specific word type, read on.<br>Specifically, it is {3}.<br>{4}<br><b>{1}</b> is the abbreviation for this.<br>The tool will only pay attention to the options you specify for {2}s in step 2. Be careful to follow any specific instructions for {1}s in step 2.".format(lemma, possible[0], generic, explanation[possible[0]]["short"], explanation[possible[0]]["technical"])
    else:
