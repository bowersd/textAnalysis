...
if __name__ == "__main__":
    ...
    for dl in diffloci:
        #print(versions["old"][dl[0]]["m_parse_lo"][dl[1]])
        if not any([versions["new"][dl[0]]["m_parse_lo"][dl[1]].endswith('+?'), versions["old"][dl[0]]["m_parse_lo"][dl[1]].endswith('+?')]): difftypes["change"].append(dl)
        elif versions["old"][dl[0]]["m_parse_lo"][dl[1]].endswith('+?'): difftypes["gain"].append(dl)
    ...
    for dt in difftypes: 
        ...
        for v in versions:
            with open("diff_check_{0}_{1}_{2}.txt".format(dt, v, date.today()), 'w') as file_out:
                for locus in h: 
                    ...
                    #for p in padded: print(p)#file_out.write(p+"\n")
                    start = 0
                    stop = 0
                    while stop < len(padded[0]):
                        while stop < len(padded[0]) and len(" ".join(padded[0][start:stop])) < 100:
                            stop += 1
                        for p in padded: 
                            file_out.write(" ".join(p[start:stop])+'\n')
                        start = stop
                        if stop < len(padded[0])-1: file_out.write('\n')
        ...
        with open("diff_check_{0}_{1}_{2}_report.txt".format(dt, v, date.today()), 'w') as report:
            ...
            for locus in h:
                ...
                if dt == "gain" or dt == "change": report.write("Credit/blame: {0}".format(versions["new"][locus[0]]["analysis_src"][locus[1]][-1])+'\n')
                if dt == "lose": report.write("Credit/blame: {0}".format(versions["old"][locus[0]]["analysis_src"][locus[1]][-1])+'\n')
