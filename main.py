...
def parse_words(event):
    ...
    for l in lemmata:
        try: gloss = gdict[l]
            ...
            gloss = "?"
        tinies.append("'"+gloss+"'")
    padded = pad(["Original Text:"] + sep_punct(freeNish.lower(), True).split(), ["Narrow Analysis:"] + m_parse_lo, ["Broad Analysis:"] + m_parse_hi, ["Dictionary Header:"] + lemmata, ["Terse Translation:"] + tinies)
